...
from debate_for_ai_alignment.pipelines.debate.models import NaiveJudgeResults, ResponseModel, \
    ResponseModelWithLogProb
from debate_for_ai_alignment.pipelines.debate.utils import _get_top_log_probs_for_answer, get_context_from_article
...
async def run_naive(article: UniqueSet, question_idx:int, is_correct_option_first: bool, llm_config) -> NaiveJudgeResults:
    logger = getLogger(__name__)
    logger.info(f"Running naive judge for question {question_idx} in article {article.set_unique_id} and is_correct_option_first={is_correct_option_first}.")
