...
from debate_for_ai_alignment.pipelines.debate.naive import run_naive
from debate_for_ai_alignment.pipelines.debate.unstructured_debate import (
    run_unstructured_debate,
)
from debate_for_ai_alignment.pipelines.debate.structured_debate import (
...
def run_consultancy_for_unique_set(unique_set: Dict, llm_config: Dict) -> Dict:
    unique_set = UniqueSet(**unique_set)
    return asyncio.run(
        _run_consultancy_for_unique_set(unique_set, llm_config)
    ).model_dump()
...
def run_unstructured_debate_for_unique_set(unique_set: Dict, llm_config: Dict) -> Dict:
    unique_set = UniqueSet(**unique_set)
    return asyncio.run(
        _run_unstructured_debate_for_unique_set(unique_set, llm_config)
    ).model_dump()
...
def run_structured_debate_for_unique_set(unique_set: Dict, llm_config: Dict) -> Dict:
    unique_set = UniqueSet(**unique_set)
    return asyncio.run(
        _run_structured_debate_for_unique_set(unique_set, llm_config)
    ).model_dump()


async def _run_expert_for_unique_set(
    unique_set: UniqueSet, llm_config: Dict
) -> UniqueSetExpertJudgeResults:
    expert_result_futures = []
    for question_idx in range(len(unique_set.questions)):
        for is_correct_option_first in [True, False]:
            expert_result_futures.append(
                run_expert(
                    unique_set, question_idx, is_correct_option_first, llm_config
                )
            )
    ...
    return UniqueSetExpertJudgeResults(
        unique_set_id=unique_set.article_id, results=expert_results
    )


async def _run_naive_for_unique_set(
    unique_set: UniqueSet, llm_config: Dict
) -> UniqueSetNaiveJudgeResults:
    naive_result_futures = []
    for question_idx in range(len(unique_set.questions)):
        for is_correct_option_first in [True, False]:
            naive_result_futures.append(
                run_naive(unique_set, question_idx, is_correct_option_first, llm_config)
            )
    ...
    return UniqueSetNaiveJudgeResults(
        unique_set_id=unique_set.article_id, results=naive_results
    )


async def _run_consultancy_for_unique_set(
    unique_set: UniqueSet, llm_config: Dict
) -> UniqueSetConsultancyResults:
    ...
    return UniqueSetConsultancyResults(
        unique_set_id=unique_set.article_id, results=consultancy_results
    )


async def _run_unstructured_debate_for_unique_set(
    unique_set: UniqueSet, llm_config: Dict
) -> UniqueSetUnstructuredDebateResults:
    ...
    unstructured_debate_results = await asyncio.gather(
        *unstructured_debate_result_futures
    )
    return UniqueSetUnstructuredDebateResults(
        unique_set_id=unique_set.article_id, results=unstructured_debate_results
    )


async def _run_structured_debate_for_unique_set(
    unique_set: UniqueSet, llm_config: Dict
) -> UniqueSetStructuredDebateResults:
    ...
    return UniqueSetStructuredDebateResults(
        unique_set_id=unique_set.article_id, results=structured_debate_results
    )
